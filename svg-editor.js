 ...
 *
 * Edited by Jonathan Breitbart, Hiroki Terashima, & Geoff Kwan for WISE4 integration
 */
...
// 				});
	...
	var toolButtonClick = function(button, fadeFlyouts) {
		...
		if ($(button).attr('id') != "tool_image"){
			$('.tools_select').fadeOut(fadeFlyouts); // fade stamp selector (wise4)
		};
		$('#styleoverrides').text('');
		$('.tool_button_current').removeClass('tool_button_current').addClass('tool_button');
	...
	var clickFHPath = function() {
		...
		// change cursor to crosshair (wise4) - indicates to students that drawing is active
		$('#styleoverrides').text('#svgcanvas svg{cursor:crosshair}');
	};
	...
	var Actions = function() {
		...
		var tool_buttons = [
		    //Removed certain keystroke events from student view (wise4)
...
(function() {
	...
	this.SVGEditor = function( jsonURL ) {
		this.init( jsonURL );
	};
	...
	SVGEditor.prototype = {
			...
			loadModules: function(jsonfilename, context) {
